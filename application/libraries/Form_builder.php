...
class Form_builder {
    ...
    function build_form_horizontal($options, $data_source = array()) {
        ...
        foreach ($options as $elm_options) {
          ...
          if ($counterDiv % 4 == 0 && $counterDiv!=0) {
            $this->print_string .= $this->_preAddDivRow();
          }
                ...
                switch ($this->func) {
                    ...
                    case 'form_radio':
                        ...
                        foreach ($all_elm_options['options'] as $elm_suboptions) {
                            ...
                            $this->print_string .= '<label >';
                            $this->print_string .= $this->_build_input(FALSE);
                            $this->print_string .= ($this->elm_options['label'] === '') ? '&nbsp;' : $this->elm_options['label'].'</label><br><br>'; // Place a nbps to keep the radio button / checkbox aligned with the main label
                        }
            ...
            if ($counterDiv % 4 == 0  && $counterDiv!=0) {
              //$this->print_string .= $this->_postAddDivRow();
            }
     ...
     * @return form elements+wrappers HTML
     */
    function get_form_fields($sector_id='',$field_group='') {
        $CI =& get_instance();
        $CI->load->model('tbl_generic_model');
        $query = "SELECT rfbf.*
                      FROM `web_registration_form_builder_relation` rfbr,
                      web_registration_form_builder_fields rfbf
                      WHERE rfbr.sector_id=".$sector_id."
                      AND rfbf.rfbf_id = rfbr.rfbf_id
                      AND rfbf.field_group = '".$field_group."'
                      ORDER BY rfbf.field_showorder ASC;
                      ";
          $field_data  = $CI->tbl_generic_model->ExecuteQuery($query);

          $formData = array();

          foreach ($field_data as $key => $value) {

              $arr = array(       'id' => $value->field_id,
                                  'placeholder' => $value->field_placeholder,
                                  'type'=>$value->field_type,
                                  'class'=>$value->field_class,
                                  'value'=>$value->field_value,
                                  'label'=>$value->field_label,
                                  'name'=> $value->field_name,  
                                  'options'=> $value->field_options,
                                  'unique'=>$value->field_unique,            
                                  
                          );

              if($value->field_validation!=''){
                $validation_arr = explode('|', $value->field_validation);
                foreach ($validation_arr as $k => $v) {
                  $validationkeyval = explode(':', $v);
                  for ($i=0; $i < count($validationkeyval) ; $i++) { 
                    $arr[$validationkeyval[$i]] = isset($validationkeyval[$i+1]) ? $validationkeyval[$i+1] : '';
                  }
                }
              }

              array_push($formData,$arr);
          }
          return $formData;
      }
        ...
        /*if ($this->input_addons['exists']) {
            if (!empty($this->input_addons['post_html'])) {
                ...
                foreach ($this->input_addons['post'] as $post_addon) {
                    $ret_string .= '<span class="spanError">' . $post_addon . '</span>';
                }
            }
            $ret_string .= '</div>';
        }*/
        $ret_string .= '<span class="help-block with-errors spanError"></span>';
        return $ret_string;
    }
    ...
    private function _pre_elm() {
        return '<div class="col-lg-3"><div class="' . $this->config['default_input_container_class'] . '">';
    }

    private function _post_elm() {
      //print_r($this->elm_options);
      if (array_key_exists("required",$this->elm_options)){
      //if($this->elm_options['required']){
        $post_str = '</div></div>';
      }else{
        $post_str =  '</div></div>';
      }
    ...
    private function _preAddDivRow() {
      return '</div><div class="row">';        
    }

    private function _postAddDivRow() {
      return '</div>';        
    }
