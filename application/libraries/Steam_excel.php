...
class Steam_excel extends PHPExcel {
    public $fileName;
    public $objPHPExcel;
    public $objExcel;
            ...
            foreach ($data['meter']['data'] as $keyD => $valueD) {
                ...
                foreach ($meterDet as $keyDe => $valueDe) {
                    if($keyDe != 0){
                        $con = 0;
                        $type = '';
                        if($keyDe < 23 && $keyDe%2 == 1){
                            $meterId = isset($valueDe['meter_id'])?$valueDe['meter_id']:0;
                            $type = isset($valueD[$valueDe['meter_id']]['type'])?$valueD[$valueDe['meter_id']]['type']:'';
                            $meterRed = isset($valueD[$valueDe['meter_id']]['meterRed'])?$valueD[$valueDe['meter_id']]['meterRed']:0;
                            $this->objExcel->setActiveSheetIndex($sheetIndex)->setCellValueByColumnAndRow($col, $row, $meterRed);
                            $col = $col + 1;
                            $con = isset($valueD[$valueDe['meter_id']]['con'])?$valueD[$valueDe['meter_id']]['con']:0;
                            $this->objExcel->setActiveSheetIndex($sheetIndex)->setCellValueByColumnAndRow($col, $row, $con);
                            $col = $col + 1;
                            if($type == 'gen'){
                               $genArr[$meterId] = $con;
                            }elseif(($type == 'dist' || $type == 'main')){
                                if(!in_array($meterId, array(6,8))){
                                    $distArr[$meterId] = $con;
                                }
                            }
                        }elseif($keyDe >= 23 && $keyDe < 28){
                            $this->objExcel->setActiveSheetIndex($sheetIndex)->setCellValueByColumnAndRow($col, $row, $con);
                            $col = $col + 1;
                        }elseif($keyDe == 28){
                            $con = array_sum($genArr) - array_sum($distArr);
                            $this->objExcel->setActiveSheetIndex($sheetIndex)->setCellValueByColumnAndRow($col, $row, $con);
                            $col = $col + 1;
                            if($con  < 0){
                                $this->_setStyle('AC'.$row.':AC'.$row , 'FFFFFF', false, 11, 'FF0000');
                            }
                        }
                    }
                    $this->_setStyle('A'.$row.':AB'.$row , 'FFFFFF', false, 11);
                }
                $row++;
            }
        }
