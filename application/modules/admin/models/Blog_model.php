...
class Blog_model extends CI_Model {
    ...
    public function getAllData(){
        $this->db->select('B.*, BREV.title, BREV.short_desc, BREV.is_status,
        BREV.created_date, BIMG.image_path, BIMG.image_alt');
        $this->db->from('blogs B');
        $this->db->join('blog_revisions BREV','BREV.blog_revision_id = B.blog_revision_id','INNER');
        $this->db->join('blog_images BIMG','BIMG.blog_id = B.blog_id AND BIMG.orders = 1 AND BIMG.is_status != "delete"','INNER');
