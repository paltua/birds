...
class Reportsteam extends MY_Controller {
	...
	public function index(){
		...
		if(isset($_POST['endDate']) && isset($_POST['meter'])){
			...
			$getAnomalyHistoryData = $this->fm_model->getAnomalyHistoryDataByDate($this->endDateTime,$_POST['meter']);
			
			if(isset($getAnomalyHistoryData) && count($getAnomalyHistoryData)==0){

				// Get Last End Date
				$getLastHistoryDate = $this->fm_model->getLastHistoryDateData($_POST['meter']);
				...
				for ( $i = strtotime($this->startDateTime); $i <= strtotime($this->endDateTime); $i = $i + 86400 ) {
					...
					if(strtotime($eDate)<=strtotime($this->endDateTime)){
						...
						$this->data['last15DataSet'] = $this->fm_model->getLastDateDataTypeWise($_POST['meter'],$sDate ,$eDate);				
				...
				$meterDetails = $this->fm_model->getMeterDetails($_POST['meter']);
		...
		if(count($newArray)>0){
			foreach ($newArray as $endDate => $dateValue) {
				...
				foreach ($dateValue as $meterId => $metervalue) {
					...
					$getHistoryData = $this->fm_model->getAllHistoryDateData($meterId); 
					...
					if($pressureValue!='' && $temperatureValue!='' && $flowValue!=''){
						$oo = $this->fm_model->insertInAnomalyHistory($meterId,$pressureValue,$temperatureValue,$flowValue,$endDate,$_SESSION['user_id']);
					}else{
						break;
					}							
