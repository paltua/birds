...
class Ems_model extends CI_Model {
    ...
    public function getMeterDetails($meterId){
    	$sql = "SELECT * FROM master_device WHERE device_id = '".$meterId."'";
    	return $this->db->query($sql)->result();
    }
...
*/
	...
	public function insertInAnomalyHistory($meter_id,$KW,$PF,$Amps,$Volt,$HZ,$end_date_time,$user_id){

		$now = date('Y-m-d H:i:s');

		$sql = "INSERT INTO anomaly_history_data (meter_id, KW, PF, Amps, Volt, HZ, end_date_time, user_id, status, created_on)
				VALUES ('".$meter_id."', '".$KW."', '".$PF."', '".$Amps."', '".$Volt."', '".$HZ."', '".$end_date_time."', '".$user_id."', 'ACTIVE', '".$now."')";
