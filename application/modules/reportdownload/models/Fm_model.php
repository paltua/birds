...
class Fm_model extends CI_Model {
    ...
    public function getTTL_flow_newDateDataTypeWise($date = ''){
		...
		$sql = "SELECT * FROM steam_meter_data_cfm SMD 
				LEFT JOIN steam_meter SM 
				ON SM.meter_id = SMD.meter_id 
				WHERE SMD.end_date_time = '".$date."'";
	...
	public function getCfmData($startDate = '', $endDate = ''){
		$sql = " SELECT 
				    ...
				    AMD.data_cfm,
				    (SELECT data_cfm FROM  steam_meter_data_cfm AMD1 WHERE AMD1.end_date_time = DATE_ADD(AMD.end_date_time, INTERVAL 1 DAY) AND AM.meter_id= AMD1.meter_id) next_data_cfm
				FROM
				    steam_meter AM
				        LEFT JOIN
				    steam_meter_data_cfm AMD ON AMD.meter_id = AM.meter_id
				WHERE
				    AMD.end_date_time BETWEEN '".$startDate."' AND '".$endDate."'
				        AND DATE_FORMAT(AMD.end_date_time, '%H:%i:%s') = '00:00:00'  
				ORDER BY AMD.end_date_time DESC, AMD.meter_id ASC";
		return $this->fm_db->query($sql)->result();	
	}
	...
	public function getLastDateDataTypeWise($startDate = '', $endDate = ''){
		$sql = "SELECT * FROM steam_meter_data SMD LEFT JOIN steam_meter SM ON SM.meter_id=SMD.meter_id WHERE SMD.end_date_time >= '".$startDate."' AND SMD.end_date_time < '".$endDate."' ORDER BY SM.type";
		return $this->fm_db->query($sql)->result();	
	}
	...
	public function getMeterDetails(){
		$sql = "SELECT * FROM steam_meter AM WHERE AM.report_excel_position != '0' ORDER BY AM.report_excel_position ASC";
		return $this->fm_db->query($sql)->result();
	}

	public function getLastDate(){
		$sql = "SELECT date_format(DATE_SUB(last_cron_run_date, INTERVAL 1 DAY),'%Y-%m-%d') max FROM agg_cron_hist WHERE id=1";
		return $this->fm_db->query($sql)->result();
	}
