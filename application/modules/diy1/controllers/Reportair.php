			...
			for ( $i = strtotime($this->startDateTime); $i <= strtotime($this->endDateTime); $i = $i + 86400 ) {
			  $thisDate = date( 'Y-m-d H:i:s', $i ); 
				...
				if(strtotime($eDate)<=strtotime($this->endDateTime)){
					...
					$this->data['last15DataSet'] = $this->air_model->getLastDateDataTypeWise($sDate ,$eDate);				
					...
					foreach ($value123 as $key => $value) {
						foreach ($value as $key1 => $value1) {
							foreach ($value1['meter'] as $key2 => $value2) {
								$newArray[$key2]['meter'] = $value2['name'];
								$newArray[$key2]['type'] = $key1;
								$newArray[$key2]['P_pressure'][$key] = $value2['P_pressure'];
								$newArray[$key2]['T_temp'][$key] = $value2['T_temp'];
								$newArray[$key2]['flow'][$key] = $value2['flow'];
							}
						}
					}
				}
			...
			if(count($newArray1)>0){
				foreach ($newArray1 as $meterId => $metervalue) {
					...
					if(isset($metervalue['P_pressure']) && count($metervalue['P_pressure'])>0){
						...
						// Call DIY1 API
						$pApiData = $this->makeAPIcall($pArray,'B');

						$pApiDataJsonDecode = json_decode($pApiData,true);
						
						$resultArray[$metervalue['type']][$meterId]['meter']['P_pressure'] = array();
						if(isset($pApiDataJsonDecode['status']) && $pApiDataJsonDecode['status']=='true' && isset($pApiDataJsonDecode['message']['data']['B'])){
							$resultArray[$metervalue['type']][$meterId]['meter']['P_pressure'] = $pApiDataJsonDecode['message']['data']['B'];
						}
					...
					if(isset($metervalue['T_temp']) && count($metervalue['T_temp'])>0){
						...
						// Call DIY1 API
						$tApiData = $this->makeAPIcall($tArray,'B');
					
						$tApiDataJsonDecode = json_decode($tApiData,true);
						
						$resultArray[$metervalue['type']][$meterId]['meter']['T_temp'] = array();
						if(isset($tApiDataJsonDecode['status']) && $tApiDataJsonDecode['status']=='true' && isset($tApiDataJsonDecode['message']['data']['B'])){
							$resultArray[$metervalue['type']][$meterId]['meter']['T_temp'] = $tApiDataJsonDecode['message']['data']['B'];
						}
					}	
					// Flow
					if(isset($metervalue['flow']) && count($metervalue['flow'])>0){
						...
						// Call DIY1 API
						$fApiData = $this->makeAPIcall($fArray,'B');
						
						$fApiDataJsonDecode = json_decode($fApiData,true);
						
						$resultArray[$metervalue['type']][$meterId]['meter']['flow'] = array();
						if(isset($fApiDataJsonDecode['status']) && $fApiDataJsonDecode['status']=='true' && isset($fApiDataJsonDecode['message']['data']['B'])){
							$resultArray[$metervalue['type']][$meterId]['meter']['flow'] = $fApiDataJsonDecode['message']['data']['B'];
						}
					}
				}
			}
