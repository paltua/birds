...
class Report extends MY_Controller {
	...
	private function geCalculationResult_cpp(){
		...
		//var_dump($this->data['last15DataSet_cpp']);
		$a = array();
		foreach ($this->data['last15DataSet_cpp'] as $keyw => $valuew) {
			if($valuew->meter_id==2){
				$a[$valuew->end_date_time] = $valuew->flow;
			}
	...
	private function _getTotalTypeWise(){
		...
		if(count($this->data['last15DataSet'])){
			...
			foreach ($this->data['last15DataSet'] as $key => $value) {
				...
				if(!isset($data[$value->type]['meter'][$value->meter_id]['P_pressure_count'])) $data[$value->type]['meter'][$value->meter_id]['P_pressure_count'] = 0;
				...
				if(!isset($data[$value->type]['meter'][$value->meter_id]['T_temp_count'])) $data[$value->type]['meter'][$value->meter_id]['T_temp_count'] = 0;
			...
			if(count($data) > 0){
				foreach ($data as $key1 => $value1) {
					foreach($value1['meter'] as $key2 => $val2){
						...
						if(isset($val2['flow_count']) && $val2['flow_count']>0 && isset($val2['flow'])){
						...
						if(isset($val2['P_pressure']) && isset($val2['P_pressure_count']) && $val2['P_pressure_count']>0){
						...
						if(isset($val2['T_temp_count']) && $val2['T_temp_count']>0 && isset($val2['T_temp'])){
			...
			if(count($retData)>0){
				...
				foreach($retData as $kk=>$vv){
					foreach($vv['meter'] as $kk1=>$vv1){
						...
						if(isset($vv1['P_pressure']) && isset($vv1['flow'])){
						...
						if(isset($vv1['T_temp']) && isset($vv1['flow'])){
						...
						if(isset($vv1['steam_enthalpy']) && isset($vv1['flow'])){
    		...
    		foreach ($meterdata as $mkey => $mvalue) {
 				...
 				if($mvalue->meter_id!=6 && $mvalue->meter_id!=8){
	    			if($this->data['dayCount'] == 1){
		    			...
		    			//$selected = 'DATE_FORMAT(SMD.end_date_time, "%Y-%m-%d") end_date_time, AVG(NULLIF(SMD.T_temp,0)) T_temp, AVG(NULLIF(SMD.P_pressure,0)) P_pressure, AVG(NULLIF(SMD.TTL_flow,0)) TTL_flow, AVG(NULLIF(SMD.flow,0)) flow, AVG(NULLIF(SMD.steam_enthalpy,0)) steam_enthalpy,AVG(NULLIF(SMD.steam_heat_content,0)) steam_heat_content';

		    			$selected = 'DATE_FORMAT(SMD.end_date_time, "%Y-%m-%d") end_date_time, (SUM(NULLIF(SMD.T_temp,0) * NULLIF(SMD.flow,0)) / SUM(NULLIF(SMD.flow,0))) T_temp, (SUM(NULLIF(SMD.P_pressure,0) * NULLIF(SMD.flow,0)) / SUM(NULLIF(SMD.flow,0))) P_pressure, AVG(NULLIF(SMD.TTL_flow,0)) TTL_flow, AVG(NULLIF(SMD.flow,0)) flow, (SUM(NULLIF(SMD.steam_enthalpy,0) * NULLIF(SMD.flow,0)) / SUM(NULLIF(SMD.flow,0))) steam_enthalpy,AVG(NULLIF(SMD.steam_heat_content,0)) steam_heat_content';
