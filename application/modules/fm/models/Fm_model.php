...
class Fm_model extends CI_Model {
    ...
    public function getMeterName($mid){
    	$sql = "SELECT * FROM steam_meter WHERE meter_id = '".$mid."'";
    	return $this->fm_db->query($sql)->result();
    }

    public function getMeterTypeWise($type='gen'){
    	$sql = "SELECT * FROM steam_meter WHERE type = '".$type."'";
    	return $this->fm_db->query($sql)->result();
    }

    public function getMeterType($mid){
    	$sql = "SELECT type FROM steam_meter WHERE meter_id = '".$mid."'";
    	return $this->fm_db->query($sql)->result();
    }
	...
	public function getLastMinuteDateDataTypeWise($date = ''){
		/*$sql = "SELECT * FROM steam_meter_data SMD LEFT JOIN steam_meter SM ON SM.meter_id=SMD.meter_id WHERE SMD.end_date_time >= '".$startDate."' AND SMD.end_date_time < '".$endDate."' ORDER BY SM.type";*/

		$sql = "SELECT * FROM steam_meter_data SMD LEFT JOIN steam_meter SM ON SM.meter_id=SMD.meter_id WHERE SMD.end_date_time = '".$date."'";
	...
	public function getTTL_flow_newDateDataTypeWise($date = ''){
		...
		$sql = "SELECT * FROM steam_meter_data_cfm SMD LEFT JOIN steam_meter SM ON SM.meter_id=SMD.meter_id WHERE SMD.end_date_time = '".$date."'";
	...
	public function getSteamMeterDataDayWiseParam($meter_id = 0, $select = '*', $startDate = '', $endDate = ''){
		$sql = "SELECT ".$select." FROM steam_meter_data SMD
				LEFT JOIN steam_meter SM ON SM.meter_id=SMD.meter_id
				WHERE SMD.meter_id = ".$meter_id." 
				AND SMD.end_date_time >= '".$startDate."' 
				AND SMD.end_date_time < '".$endDate."' 
				GROUP BY DATE_FORMAT(SMD.end_date_time, '%Y%m%d')
				ORDER BY SMD.id DESC,SMD.end_date_time DESC ";
