			...
			if(count($newArray1)>0){
				foreach ($newArray1 as $meterId => $metervalue) {
					...
					if(isset($metervalue['P_pressure']) && count($metervalue['P_pressure'])>0){
						...
						$arrPaddiInfo = array("data_key" => "P_pressure","given_high" => "","given_low" => "", "running_info" => array());
						...
						if(isset($pApiDataJsonDecode['status']) && $pApiDataJsonDecode['status']=='SUCCESS' && isset($pApiDataJsonDecode['data'])){
							$resultArray[$metervalue['type']][$meterId]['meter']['P_pressure'] = $pApiDataJsonDecode['data'];
						}
					...
					if(isset($metervalue['T_temp']) && count($metervalue['T_temp'])>0){
						...
						$arrTaddiInfo = array("data_key" => "T_temp","given_high" => "","given_low" => "", "running_info" => array());
						...
						if(isset($tApiDataJsonDecode['status']) && $tApiDataJsonDecode['status']=='SUCCESS' && isset($tApiDataJsonDecode['data'])){
							$resultArray[$metervalue['type']][$meterId]['meter']['T_temp'] = $tApiDataJsonDecode['data'];
						}
					}	
					// Flow
					if(isset($metervalue['flow']) && count($metervalue['flow'])>0){
						...
						$arrFaddiInfo = array("data_key" => "flow","given_high" => "","given_low" => "", "running_info" => array());
						...
						if(isset($fApiDataJsonDecode['status']) && $fApiDataJsonDecode['status']=='SUCCESS' && isset($fApiDataJsonDecode['data'])){
							$resultArray[$metervalue['type']][$meterId]['meter']['flow'] = $fApiDataJsonDecode['data'];
						}
					}
				}
			}
