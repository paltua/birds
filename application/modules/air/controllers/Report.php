...
class Report extends MY_Controller {
	...
	public function index(){
		...
		if(isset($_POST['startDate'])){
            ...
            $this->data['last15DataSet_KWsumproductCFM'] = $this->_getCompRelKW_forSumproductCFM($startDate,$endDate);

            //echo "<pre>";
            //var_dump($this->data['last15DataSet_KWsumproductCFM']);
...
*/
    ...
    private function _getCompRelKW_CFMimestampTemp($meter_id=NULL,$startDate,$endDate,$dayCount){
    ...
    public function getMeterChart_forKWCFM_temp(){
        ...
        // Get KW value according 15 mint timestamp interval
        //$dataFinalArray_KW = $this->_getCompRelKW_CFMimestamp($meter_id,$startDate,$endDate,$this->data['dayCount']); // Updated 17.01.2018

        $dataFinalArray_KW = $this->_getCompRelKW_CFMimestampTemp($meter_id,$startDate,$endDate,$this->data['dayCount']);       
        //echo "<pre>";
        //var_dump($dataFinalArray_KW);
        ...
        if($this->data['dayCount'] == 1){
            ...
            //echo  $endDate;
            //echo $selected;
            $selected = 'AM.name,DATE_FORMAT(AMD.end_date_time, "%Y-%m-%d %H:%i:%s") end_date_time,(NULLIF(AMD.flow,0)) flow';
            $dataFinalArray_CFM = $this->air_model->getAirMeterDataDayShift($meter_id, $selected, $startDate, $endDate);
        }
        ...
        if(isset($dataFinalArray_CFM) && count($dataFinalArray_CFM)>0){
            if($this->data['dayCount'] == 1){
                ...
                foreach ($dataFinalArray_CFM as $inkey => $invalue) {
                    ...
                    if(!isset($tKW_CFM[date('Y-m-d',strtotime($invalue->end_date_time))])) $tKW_CFM[date('Y-m-d',strtotime($invalue->end_date_time))] = 0;
                    if(!isset($tCFMsum[date('Y-m-d',strtotime($invalue->end_date_time))])) $tCFMsum[date('Y-m-d',strtotime($invalue->end_date_time))] = 0;
                    if(!isset($tKWsum[date('Y-m-d',strtotime($invalue->end_date_time))])) $tKWsum[date('Y-m-d',strtotime($invalue->end_date_time))] = 0;

                    if(isset($dataFinalArray_KW[$invalue->end_date_time]) && isset($invalue->end_date_time)){
                        $tKW_CFM[date('Y-m-d',strtotime($invalue->end_date_time))] = (isset($invalue->flow)) ? (($dataFinalArray_KW[$invalue->end_date_time] * $invalue->flow) + $tKW_CFM[date('Y-m-d',strtotime($invalue->end_date_time))]) : 0;

                        /*var_dump(date('Y-m-d',strtotime($invalue->end_date_time)));
                        var_dump($invalue->flow);*/
                        //var_dump($invalue->end_date_time);
                        //var_dump($invalue->flow);
                        
                        if($dataFinalArray_KW[$invalue->end_date_time]>0){

                            $tCFMsum[date('Y-m-d',strtotime($invalue->end_date_time))] = ($invalue->flow + $tCFMsum[date('Y-m-d',strtotime($invalue->end_date_time))]);

                            $tKWsum[date('Y-m-d',strtotime($invalue->end_date_time))] = ($dataFinalArray_KW[$invalue->end_date_time] + $tKWsum[date('Y-m-d',strtotime($invalue->end_date_time))]);
                        }
                    }
                ...
                if(isset($tKW_CFM) && count($tKW_CFM)>0){
                    foreach ($tKW_CFM as $timemmkey => $timemmvalue) {
                        $kwData = 0;
                        $cfmData = 0;
                        if(isset($tCFMsum[$timemmkey]) && $tCFMsum[$timemmkey]>0){
                            $cfmData = ($timemmvalue / $tCFMsum[$timemmkey]);
                        }
                        if(isset($tKWsum[$timemmkey]) && $tKWsum[$timemmkey]>0){
                            $kwData = ($timemmvalue / $tKWsum[$timemmkey]);
                        }
                        if(isset($kwData) && $kwData>0){
                            $dataFinalArray[$timemmkey] = ($cfmData/$kwData);
                        }
    ...
    private function _getCompRelKW_forSumproductCFM($startDate,$endDate){
        //$deviceIds = '48,46,47,44,43,45,51,52,53,56,57,49,50,55';
        //$data = array(1 => 48, 2 => 47, 3=>46, 4=>44,5=>43,6=>45, 7=>51, 8=>52, 9=>53,10=> 56,11=>0,12=>57,13=>49,14=>50,15 => 55);
        //echo "<pre>";
        //var_dump($this->data['last15DataSet']);
        ...
        if(isset($this->data['last15DataSet']) && count($this->data['last15DataSet'])>0){
            foreach ($this->data['last15DataSet'] as $inddkey => $inddvalue) {
                //var_dump($inddvalue);
                if(isset($inddvalue->type) && isset($inddvalue->meter_id) && isset($inddvalue->end_date_time) && isset($inddvalue->flow)){
                    $cfmArray[$inddvalue->meter_id][$inddvalue->end_date_time] = $inddvalue->flow;
                }
            }
        }
        ...
        // Start Old Code Average Of KW 17.01.2018
        /*$retData = array();
        foreach ($data as $key => $value) {
            if(isset($tempSql[$value])){
                $retData[$key] = array(round((array_sum($tempSql[$value])/count($tempSql[$value])),2),$value);
        ...
        // End Old Code Average Of KW 17.01.2018
        // Start New Code Sumproduct Of KW 17.01.2018
        //echo "<pre>";
        $tempVals = array();
        $tempVals_sumcfm = array();
        $tempVals_sumkw = array();
        foreach ($data as $key => $value) {
            if(isset($tempSql[$value]) && isset($cfmArray[$key])){
                //var_dump($tempSql[$value]);
                foreach ($tempSql[$value] as $timesskey => $timessvalue) {
                    if(!isset($tempVals[$key][0])) $tempVals[$key][0] = 0;
                    if(!isset($tempVals[$key][1])) $tempVals[$key][1] = 0;
                    if(!isset($tempVals_sumcfm[$key])) $tempVals_sumcfm[$key] = 0;
                    if(!isset($tempVals_sumkw[$key])) $tempVals_sumkw[$key] = 0;

                    if(isset($cfmArray[$key][$timesskey])){


                        $tempVals[$key][0] = (($timessvalue * $cfmArray[$key][$timesskey]) + $tempVals[$key][0]);
                        $tempVals[$key][1] = $value;
                        if($timessvalue>0){
                            $tempVals_sumcfm[$key] = ($cfmArray[$key][$timesskey] + $tempVals_sumcfm[$key]);
                            $tempVals_sumkw[$key] = ($timessvalue + $tempVals_sumkw[$key]);
                        }
                ...
                $tempVals[$key] = array('DNA',$value);
            }
        }
        $retData = array();
        //echo "<pre>";
        //var_dump($tempVals[3]);
        //var_dump($tempVals_sumcfm[3]);
        
        //var_dump($tempVals_sumkw[3]);

        if(isset($tempVals) && count($tempVals)>0){
            foreach ($tempVals as $didkey => $didvalue) {
                if(isset($didvalue[1]) && isset($didvalue[0]) && $didvalue[0]!='DNA'){
                    //var_dump($didvalue);
                    $kwData = 0;
                    $cfmData = 0;
                    if(isset($tempVals_sumcfm[$didkey])){
                       $cfmData =  ($didvalue[0] / $tempVals_sumcfm[$didkey]); 
                    }
                    if(isset($tempVals_sumkw[$didkey])){
                       $kwData =  ($didvalue[0] / $tempVals_sumkw[$didkey]); 
                    }
                    
                    if(isset($kwData) && $kwData>0){
                        $retData[$didkey] = array(round(($cfmData / $kwData),3),$didvalue[1]);
                        if($didkey==3){
                            /*var_dump($didvalue[0]);
                            var_dump($tempVals_sumcfm[$didkey]);
                            var_dump($tempVals_sumkw[$didkey]);
                            var_dump($cfmData);
                            var_dump($kwData);
                            var_dump($retData[$didkey]);*/
                        }
                    }
                    
                    
                }else{
                    $retData[$didkey] = array('DNA',$didvalue[1]);
                }
            }
        }
        // End Code Sumproduct Of KW 17.01.2018
        //echo "<pre>";
        //var_dump($retData);
        ...
        if(count($resultArray) > 0){
            ...
            foreach ($resultArray as $dateTimeKey => $dateTimeValue) {
                ...
                if(isset($dateTimeKey) && $dateTimeKey!=''){
                    ...
                    if($type == 'pres'){
                        $p = isset($dateTimeValue) ? number_format((float)round($dateTimeValue,3), 2, '.', '') : '0';
                        $f = isset($resultArray2[$dateTimeKey]) ? number_format((float)round($resultArray2[$dateTimeKey],3), 2, '.', '') : '0';
                    }else{
                        $f = isset($dateTimeValue) ? number_format((float)round($dateTimeValue,3), 2, '.', '') : '0';
                        $p = isset($resultArray2[$dateTimeKey]) ? number_format((float)round($resultArray2[$dateTimeKey],3), 2, '.', '') : '0';
                    }
            ...
            foreach ($finalArray as $key => $value) {
                ...
                if(isset($value['end_date_time']) && $value['end_date_time']!='' && isset($value['cuPercent'])){
                    ...
                    $dataSet .= '{
                        ...
                        "paramVal": "'.round($value['cuPercent'],3).'"
                    },';
    ...
    public function getMeterChartShiftForDoubleGraph(){
        ...
        $this->data['chartData'] = $dataSet;
        $this->load->view('report/chart_loading_am_line',$this->data);
    ...
    public function getMeterChartAllDayTotalGNvsDISTForDoubleGraph(){
        ...
        if($this->data['dayCount'] == 1){
            ...
            $this->load->view('report/chart_loading_am_bar',$this->data);
        }
